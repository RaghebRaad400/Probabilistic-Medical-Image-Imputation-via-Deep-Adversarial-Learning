...
def disk_image_batch(image_paths, batch_size, preprocess_fn=None, shuffle=True, num_threads=16,
    ...
    with tf.name_scope(scope, 'disk_image_batch'):
        #(delete)data_num = len(image_paths) 
        ...
        if preprocess_fn is not None:
            img = preprocess_fn(img) #change preprocess_fn(img)
            data_num = int(img.shape[0])
        ...
        if shuffle:
            ...
            img_batch = tf.train.shuffle_batch([img],
                                               ...
                                               min_after_dequeue=min_after_dequeue,
                                               num_threads=num_threads,
                                               allow_smaller_final_batch=allow_smaller_final_batch)#add enqueue_many=True,
        else:
...
class DiskImageData:
    ...
    def __init__(self, image_paths, batch_size, preprocess_fn=None, shuffle=True, num_threads=16,
                 min_after_dequeue=100, allow_smaller_final_batch=False, scope=None):
        ...
        with self.graph.as_default():
            ...
            # same with the main graph, but cpu device is ok. I don't know why...
            with tf.device('/cpu:0'):
                self._batch_ops, self._data_num = disk_image_batch(image_paths, batch_size, preprocess_fn, shuffle, num_threads,
                                                                   min_after_dequeue, allow_smaller_final_batch, scope)
            #delete shape    
...
def imwrite(image, save_dir, epoch, it_epoch, batch_epoch):#path):
    ...
    new_im_data = image*255
    path_mat='%s/Epoch_(%d)_(%dof%d).mat' % (save_dir, epoch, it_epoch, batch_epoch)#'%s/phase.mat' % (save_dir)
    sio.savemat(path_mat, {'image':image})

    image_list=[]
    for i in range(4):
        phase=image[:,:,i]
        #phase_image=Image.fromarray(phase)
        image_list.append(phase)#_image)
    ...
    for image in image_list:
        ...
        plt.axis('off')
        im=plt.imshow(image,cmap='inferno', norm=norm)
        path='%s/Epoch_(%d)_(%dof%d)_phase(%d).png' % (save_dir, epoch, it_epoch, batch_epoch, index)
        plt.savefig(path)
        index += 1
    

    #return imageio.imwrite(path, new_im)#to_range(image, 0, 255), np.uint8))
